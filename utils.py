...
prompt = '''
[label] Please judge whether there is a given relationship between the head entity and the tail entity based on the connection path between the head entity and the tail entity:
Connection path: {}
Instance to be judged: {}
Result### {}
'''.strip()

prompt_reason = '''
[rationale] Please judge whether there is a given relationship between the head entity and the tail entity based on the connection path between the head entity and the tail entity:
Connection path: {}
Instance to be judged: {}
Reason### {}
'''.strip()
...
def convert_input(batchs):
    if len(batchs) == 6:
        sentence1, sentence2, targets, num, contra_dict, cot_dict = batchs
        contra, reason = True, True
    else:
        sentence1, sentence2, targets = batchs
        contra, reason = False, False

    inputs = []
    inputs_r = []
    for sents1, sents2, tgt in zip(sentence1, sentence2, targets):
        for tdx, (sent1, sent2) in enumerate(zip(sents1, sents2)):
            result = "yes" if tgt == tdx else "no"
            if result == "yes" and contra:
                key = sent1.strip() + str(num)
                intra_label = contra_dict[key]
                result = "yes" if intra_label == "yes" else "uncertain"
            if reason:
                key = sent1.strip() + str(num)
                cot_list = cot_dict[key]
            sent1 = sent1.replace('[SEP]', '').strip()
            sent2 = " ".join(["Path {}: ".format(idx +1) + sent.replace('[SEP]', '').strip() + "." for idx, sent in enumerate(sent2)]).strip()
            if sent2 == "":
                sent2 = "None"
            if not reason:
                ipt = prompt.format(sent2, sent1, result)
                inputs.append(ipt)
            else:
                ipt = prompt.format(sent2, sent1, result)
                inputs.append(ipt)
                ...
                for r in cot_list[:1]:
                    #r = r[:130]
                    #sent2 = sent2[:300]
                    ipt_r = prompt_reason.format(sent2, sent1, r)
                    ipt_r_tmp.append(ipt_r)
                inputs_r.append(ipt_r_tmp)
    if not reason:
        return inputs
    else:
