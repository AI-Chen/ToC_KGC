...
import os
import random
from utils import load_count_dict, load_cycle, negative_relation_sampling, load_text, load_paths, load_triplets, \
    myConvert,reshape_relation_prediction_ranking_data,element_wise_cos,cal_metrics,CosineEmbeddingLoss, \
    convert_input, load_cot, load_contra_label
...
parser.add_argument('--learning_rate', type=float, default=1e-5,
                    help='learning rate')
parser.add_argument('--model', type=str, default='/srv/xiaoshuchen/MODEL/gpt2_base',
                    help='sentence transformer model name on Hugging Face website (huggingface.co)')
parser.add_argument('--tokenizer', type=str, default='/srv/xiaoshuchen/MODEL/gpt2_base',
...
parser.add_argument('--mode', type=str, default='tail', help='whether head or tail is fixed')
parser.add_argument('--seed', type=int, default=42, help='random seed')
parser.add_argument('--suffix', type=str, default="_full", help='suffix of the train file name')
...
################################################  Data Loader   ###############################################
intra_dict = load_contra_label(os.path.join(path_dir, "intra_reason.jsonl"))
cot_dict = load_cot(os.path.join(path_dir, "cot_reason.jsonl"))
...
def train():
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for step, batch in enumerate(train_data_loader):
            ...
            sentence2_intra_list = []  # intra_num, bs, neg_num, path_num, eg: 3,1,1,3 only correct
            sentence1_list = []
            for idx in range(3):
                sentence2_list = []
                sentence1_tmplist = []
                for mdx, sent in enumerate(sentence2):
                    tmp_path = []
                    tmp_path1 = []
                    for ndx, s in enumerate(sent):
                        tmp_path1.append(sentence1[mdx][ndx])
                        if idx == 0:
                            tmp_path.append(s[:3])
                        elif idx == 1:
                            tmp_path.append(s[3:])
                        elif idx == 2:
                            tmp_path.append([s[0], s[3], s[4]])
                    sentence2_list.append(tmp_path)
                    sentence1_tmplist.append(tmp_path1)
                sentence1_list.append(sentence1_tmplist)
                sentence2_intra_list.append(sentence2_list)
            optimizer.zero_grad()
            loss_list = []
            for jdx, (sentence1, sentence2) in enumerate(zip(sentence1_list, sentence2_intra_list)):
                batch = convert_input((sentence1, sentence2, targets, jdx, intra_dict, cot_dict))
                with autocast():
            ...
            tr_loss += loss.item()
            nb_tr_steps += 1
            training_pbar.update(args.batch_size)
        training_pbar.close()
        scheduler.step()
...
def validate():
    ...
    for batch in valid_data_loader:
        ...
        batch = convert_input((sentence1, sentence2, targets))
        ...
        nb_valid_steps +=1
        valid_pbar.update(args.batch_size)
    valid_pbar.close()
...
def test():
    ...
    ranking_pbar.close()
    with open(os.path.join(output_dir, "mrr_hit.txt"), mode="w") as fout:
        fout.write(f"MR: {metrics[0]}, MRR: {metrics[1]}, Hit@1: {metrics[2]}, Hit@3: {metrics[3]}, Hit@10: {metrics[4]}")
    print(f"MR: {metrics[0]}, MRR: {metrics[1]}, Hit@1: {metrics[2]}, Hit@3: {metrics[3]}, Hit@10: {metrics[4]}")
    return metrics[2]
